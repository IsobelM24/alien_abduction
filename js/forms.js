...
function sendData() {
  ...
  // radio buttons
  let radio = document.getElementsByName("ufo_type");
  for (var i = 0, length = radio.length; i < length; i++) {
    if (radio[i].checked) {
      urlEncodedDataPairs.push(
        encodeURIComponent("ufo_type") + "=" + encodeURIComponent(radio[i].value)
      );
    }
  }

  // dropdown menu
  var dropdown = form.querySelector("[name='abduction_type']");
  urlEncodedDataPairs.push(
    encodeURIComponent("abduction_type") +
      "=" +
      encodeURIComponent(dropdown.options[dropdown.selectedIndex].text)
  );
  urlEncodedDataPairs.push(
    encodeURIComponent("requests") +
      "=" +
      encodeURIComponent(form.querySelector("[name='requests']").value)
  );
  urlEncodedDataPairs.push(
